...
@export var current_upgrade_count : int:
    set(value):
        ...
        if !Engine.is_editor_hint():
            if use_upgrade_count: Events.emit_upgrade_changed(upgrade_name, current_upgrade_count)
            else: Events.emit_upgrade_changed(upgrade_name, total_amount)
...
func _ready() -> void:
    ...
    if !Engine.is_editor_hint():
        upgrade_button.pressed.connect(
            func try_upgrade():
                if !upgrades_menu.try_resource_transaction(consume_resource_id, -total_cost): return
...
func update_upgrade():
    ...
    if upgrade_cap > 0: upgrade_count_label.text = str(current_upgrade_count, "/", upgrade_cap)
    else: upgrade_count_label.text = str(current_upgrade_count)
